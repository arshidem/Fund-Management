...
const OverviewTab = ({
  ...
  // Update join handler to use toast
    ...
    try {
      ...
      if (result.success) {
        ...
        if (onParticipationChange) {
          onParticipationChange(true, result.participant);
        }
      ...
      toast.error(error.message || "Failed to join the event. Please try again.");
    } finally {
      setJoining(false);
    }
  };
  ...
  const handleLeaveClick = () => {
    setConfirmModal({
      ...
      message: `Are you sure you want to leave "${event.title}"? You'll need to join again if you change your mind.`,
  ...
  const handleConfirmLeave = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (onParticipationChange) {
          onParticipationChange(false, null);
        }
        
        toast.success("Successfully left the event!");
      } else {
        toast.error(result.message || "Failed to leave the event.");
      }
    } catch (error) {
      console.error("Error leaving event:", error);
      toast.error(error.message || "Failed to leave the event. Please try again.");
    } finally {
      ...
      <div className="space-y-3">
        ...
        {!participationStatus.isParticipating ? (
          <button
            ...
            disabled={joining || (event.status !== "ongoing" && event.status !== "published")}
            className={`w-full flex items-center justify-center gap-2 py-3 rounded-lg font-semibold transition-all duration-300 shadow-sm hover:shadow-md ${
              joining || (event.status !== "ongoing" && event.status !== "published")
          ...
          <button
            onClick={handleLeaveClick} // Fixed: Changed from handleLeaveEvent to handleLeaveClick
            disabled={leaving}
  ...
  return (
    <>
      <div className="grid md:grid-cols-2 gap-6">
            ...
            {event.estimatedBudget && (
              <div>
                <div className="flex justify-between text-sm mb-2">
                  ...
                  <span className="text-gray-900 dark:text-white">
                    {event.budgetUtilization?.toFixed(1)}%
                  </span>
                </div>
                <div className="w-full bg-gray-200 dark:bg-gray-700 h-3 rounded-full overflow-hidden">
                  <div
        ...
        {participationStatus.isParticipating &&
          participationStatus.participantData && (
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="text-center p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                  <div className="text-2xl font-bold text-green-600 dark:text-green-400 mb-1">
                    {formatCurrency(
                      participationStatus.participantData.totalContributed || 0
                    )}
                  </div>
                ...
                <div className="text-center p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                  <div className="text-2xl font-bold text-blue-600 dark:text-blue-400 mb-1">
                    {formatCurrency(
                      participationStatus.participantData.remainingAmount || 0
                    )}
                  </div>
                  <div className="text-gray-600 dark:text-gray-400 text-sm">
                    Remaining Amount
                ...
                <div className="text-center p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg">
                  <div className="text-2xl font-bold text-purple-600 dark:text-purple-400 mb-1">
                    {participationStatus.participantData.paymentPercentage?.toFixed(
                      1
                    ) || 0}
                    %
                  </div>
